...
2. **Update dashboard** — MANDATORY. Run the dashboard generator to refresh the Command Center with the latest run data:
   ```bash
   bash _opensquad/update-dashboard.sh
   ```
   This updates `_opensquad/dashboard.html` so the user sees fresh data when they open the browser shortcut.

3. **Update squad memory** — write to BOTH files (runs after Post-Completion Cleanup above):
